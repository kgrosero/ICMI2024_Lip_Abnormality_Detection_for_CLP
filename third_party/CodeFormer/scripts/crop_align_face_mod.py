...
class Image_Preprocess:
    def __init__(self):
        self.predictor = dlib.shape_predictor('weights/dlib/shape_predictor_68_face_landmarks-fbdc2cb8.dat')

    def get_landmark(self, filepath, only_keep_largest=True):
    ...
    def align_face(self, filename, fileout):
        """
        :param filename: str
        :return: PIL Image
        """
        try:
            lm = self.get_landmark(filename)
        except:
            print('No landmark', filename)
            return
        ...
        if output_size < transform_size:
            img = img.resize((output_size, output_size), PIL.Image.Resampling.LANCZOS)#PIL.Image.ANTIALIAS)
        
        img.save(fileout)
        
        return img, np.max(quad[:, 0]) - np.min(quad[:, 0])

    # def run_crop(in_dir, out_dir):
    ...
    #         out_path = os.path.join(out_dir, in_path.split("/")[-1])        
